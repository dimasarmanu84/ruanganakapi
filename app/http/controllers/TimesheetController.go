...
func (ctrl TimesheetController) Edit(c *gin.Context) {
	...
	if err := ctrl.db.Select("*, to_char(clock_in, 'HH:mi:ss') as clock_in,clock_out, total_overtime").Table("v_timesheet_child_overtime").Where("child_id::text = ?", postdata["child_id"]).Where("v_timesheet_child_overtime.date BETWEEN ? AND ?", postdata["start_date"], postdata["end_date"]).Scan(&timesheet).Error; err != nil {
